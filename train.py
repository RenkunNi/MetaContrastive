...
def add_learner_params(parser):
    ...
    parser.add_argument('--data_path', default='../../../../../fs/cml-datasets/ImageNet/ILSVRC2012/', type=str,
        help='Optional checkpoint to init the model.'
    )
    parser.add_argument('--run_id', default=None, type=str, help='run id for resume training')
    ...
    parser.add_argument('--accu_iter', default=1, type=int, help='gradient accumulation')
    ...
    parser.add_argument('--task_aug', default=False, type=bool, help='Using task augmentation if True')
...
def main():
    parser = myexman.ExParser(file=sys.argv[2])
...
def main_worker(gpu, ngpus, args):
    ...
    if args.ckpt != '':
        print("resume from ", args.ckpt)
        ckpt = torch.load(args.ckpt, map_location=device)
        model.load_state_dict(ckpt['state_dict'])

    # Data loading code
    model.prepare_data(taskaug=args.task_aug)
    ...
    while continue_training:
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if not args.eval_only:
                ...
                #optimizer.step()
                loss.backward()
                if ((batch_idx + 1) % args.accu_iter == 0) or (batch_idx + 1 == len(train_loader)):
                    optimizer.step()
                    optimizer.zero_grad()
